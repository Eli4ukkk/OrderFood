<template>
	<view style="padding-bottom: 300rpx;" class="container">
		...
		<!-- 菜 -->
		<view class="bg-white solid margin-sm padding-sm radius">
			<FoodCard v-for="item in foodList" :key="item.id" :item="item" :is-edit-module="isEditModule"
				@delete="handleFoodDeleteTap" @toggle-check="toggleFoodChecked" @touch-start="handleFoodTouchStart"
				@touch-move="handleFoodTouchMove" @touch-end="handleFoodTouchEnd"
				@touch-cancel="handleFoodTouchCancel" />
		</view>

		<AddNewFoodModel :visible="showFoodDialog" :base-url="baseUrl" :token="token"
			:category-list="categoryList" @created="requestFoodList" @type-deleted="requestCategoryList"
			@add-type="openAddCategoryDialog" @close="showFoodDialog = false" />

		<AddNewTypeModel :visible="showAddCategoryDialog" :base-url="baseUrl" :token="token"
			@created="requestCategoryList" @close="showAddCategoryDialog = false" />
...
<script>
	...
	import AddNewTypeModel from "@/components/manager/foodstatus/AddNewTypeModel.vue"
	import AddNewFoodModel from "@/components/manager/foodstatus/AddNewFoodModel.vue"
	import FoodCard from "@/components/manager/foodstatus/FoodCard.vue"
	export default {
		/**
		 * @声明组件
		 */
